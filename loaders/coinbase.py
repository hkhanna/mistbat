...
def update_from_remote():
    ...
    for account in accounts:
        # Coinbase Buys
        buys = json.loads(
            str(client.get_buys(account['id'])))['data']
        cb_resources['buys'][account['currency']] = list(filter(
            lambda buy: buy['status'] != 'canceled', buys))

        # Coinbase Sells
        sells = json.loads(
            str(client.get_sells(account['id'])))['data']
        cb_resources['sells'][account['currency']] = list(filter(
            lambda sell: sell['status'] != 'canceled', sells))
        ...
        transactions = json.loads(str(client.get_transactions(
            account['id'])))['data']
        cb_resources['transactions_filtered'][account['currency']] = list(
            filter(lambda tx:
                   tx['type'] != 'buy' and tx['type'] != 'sell'
...
def parse_events():
    ...
    types = [obs['type']
             ...
             for obs in coin_val]
    types = set(types)
    assert {'send', 'exchange_withdrawal', 'exchange_deposit', 'fiat_deposit', 'fiat_withdrawal'} == types
    ...
    for buy in buys_flat:
        ...
        assert all([fee['amount']['currency'] == 'USD'
                    for fee in buy['fees']])
        ...
        fiat_exchange = FiatExchange(
           ...
           fee_with='USD',
           fee_amount=sum([float(fee['amount']['amount'])
                           for fee in buy['fees']]),
           location_id=buy['id']
        )
    ...
    send_flat = [obs for coin_val in transactions_filtered.values()
                 for obs in coin_val
                 if obs['type'] == "send"
                 and 'to' in obs]

    for send_obs in send_flat:
        fmv = float(send_obs['native_amount']['amount']) / float(send_obs['amount']['amount'])
        send = Send(
    ...
    recv_flat = [obs for coin_val in transactions_filtered.values()
                 for obs in coin_val
                 if obs['type'] == "send"
                 and 'from' in obs]

    for recv_obs in recv_flat:
        fmv = float(recv_obs['native_amount']['amount']) / float(recv_obs['amount']['amount'])
        receive = Receive(
